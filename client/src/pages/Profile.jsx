  ...
  return (
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <p className='text-sm self-center'>
          {fileUploadError ? (
            <span className='text-red-700'>Error uploading image(image must be less than 2mb)</span>
              ...
              <span className='text-green-800'>Image Upload Successful!</span>
          ) : (
            ''
          )};
        </p>
        ...
        id='username' className='border p-3 rounded-lg' onChange={handleChange} />
        <button disabled={loading} className='bg-slate-500 text-white rounded-lg p-3 uppercase hover:opacity-90
         disabled:70'>{loading ? 'Loading...' : 'Update'}</button>
      ...
      <p className='text-red-800 mt-7'>{error ? error : ''}</p>
      <p className='text-green-800 mt-7'>{updateSuccess ? 'Success' : ''}</p>
    </div>
  );
}
